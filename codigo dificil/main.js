...
function PCPlaysV2() {
  ...
  //create three
  const copy = JSON.parse(JSON.stringify(board)); // copia lo del board y lo transforma en un arreglo
  const root = new Node(copy);/// cpy lo pasa como arguemento a un nuevo nodo para asi guardarr la tabla del padre

  console.log(root, "hhhhhh")
  processNode(root, true, 0); /// define una funcion como arguemnto root, true y  o

  console.log("final", root);

  if (pcSolutions.length > 0) { // crea un condicional donde dice que si el arreglo de pcsolution.length es mayor 0 entoces va hacer cierta cosa
    let min = 100;
    for (let i = 0; i < pcSolutions.length; i++) {
      if (pcSolutions[i].level < min) {
        min = pcSolutions[i].level;
        console.log(min, "eeee")
      }
      console.log(pcSolutions, " jjjjjjjjjj")
    }
